...
def makePlots(options, graphs, SR, outputFilename, doObs=False, bestSRXsecData = {}):
    ...
    g_xsec.SetFillStyle(3154)
    g_xsec.Draw("AL3")
    
    if not options.vertical:
        g_xsec.SetMinimum(10)
...
#    g_xsec4.Draw("l")
    ...
    # if options.gridname == "wino": g_xsec8.Draw("l3")
...
#    leg.AddEntry(g_xsec4, "2 squarks", "l")
    #if options.gridname == "wino": leg.AddEntry(g_xsec8, "#sigma_{#tilde{q}#tilde{q}} (1 #tilde{q})", "lf")
...
def main():
    ...
    parser.add_option("-f", "--filename", help="filename", default="LepPhoton8TeV_weak_NoTheoryUncertsXsecNominal_AnalysisOutput_upperlimit__1_harvest_list")
    parser.add_option("-g", "--gridname", help="name of grid (be sure to implement code!)", default="wino")
    ...
    if options.gridname != "wino" and "wino" in options.filename:
        ...
        options.filename = options.filename.replace("wino", options.gridname)
    ...
    makePlots(options, graphs, -1, outputname, doObs)
