...
def tablefragment(m, channel, signalregionslist,sampleList):
...
%%'''
  ...
  tableline += '''
MC exp. SM events             '''
  for n in m['TOTAL_MC_EXP_BKG_events']:
       tableline += " & $" + str(("%.1f" %n)) +  "$         "
  tableline +='''     \\\\
\\noalign{\\smallskip}\\hline\\noalign{\\smallskip}
%%''' 
    ...
    for name in map_listofkeys:
      if "MC_exp_events_" in name and sample in name:
        ...
        for index, n in enumerate(m[name]):
          if m['names'][index] in signalregionslist:
            tableline += " & $" + str(("%.1f" %n)) +  "$         "
            #tableline += " & $" + str(("%.1f" %n)) + " \\pm " + str(("%.1f" %m['MC_exp_err_'+sample][index])) +  "$         "
          else:
            tableline += " & $" + str(("%.1f" %n)) +  "$         "
          #tableline += " & $" + str(("%.1f" %n)) + " \\pm " + str(("%.1f" %m['MC_exp_err_'+sample][index])) +  "$         "
        tableline +='''     \\\\
